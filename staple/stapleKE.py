...
def staple(number, sequence):
    ...
    for idx, organ in enumerate(l_organs):
        ...
        if (num_nonzero_voxels_TS < 1 and num_nonzero_voxels_MR < 1 and num_nonzero_voxels_V < 1):
            ...
            result = run_staple_with_timeout(seg_stack, timeout=120, stop_event=threading.Event())
    ...
    sitk.WriteImage(itk_mask_final, os.path.join(out_path, sequence, number)+".nii")
    print(error)
    if len(error) > 0:
        with open("/data/drdcad/nicole/benchmark/errorKE.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    #staple(number="0007_0009", sequence="A_SE")
    
    
    for sequence in ['E']: #
        print(f"Starting sequence {sequence}")
        file_list = [file for file in os.listdir(f"{duke_dir}{sequence}") if file.endswith(".nii.gz")][63:]
        count = 1
